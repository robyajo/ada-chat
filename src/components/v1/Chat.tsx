...
export default function Chat({
  ...
  useEffect(() => {
      ...
      if (payload.channel === room) {
        ...
        if (payload.event === "chat.join") {
          ...
          if (username && username !== user) {
            // Only add system message if not a DM room
            if (!room.startsWith("dm_")) {
              setMessages((prev) => [
                ...prev,
                {
    ...
    socket.on("error", (err: unknown) => setError(err instanceof Error ? err.message : String(err)))
    return () => {
      leaveRoom(socket)
      disconnectChat()
  ...
  const send = useCallback(async () => {
    ...
    // Update DMs sidebar info on sending message
    if (room.startsWith("dm_")) {
      const dmUser = getDmTarget(room, user)
      if (dmUser) {
        setRecentDms((prev) => {
        ...
        <DialogContent className="rounded-2xl border-border bg-card text-slate-100 sm:max-w-[420px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              Enter Room ID or Token to join
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                className="h-11 rounded-xl border-border bg-muted/60 text-white placeholder-slate-500 focus-visible:ring-indigo-500"
                value={joinRoomId}
                onChange={(e) => setJoinRoomId(e.target.value)}
                placeholder="Paste Room ID or Token"
              />
            </div>
            {dialogError && (
              <Alert
            ...
            <Button
              className="h-11 w-full rounded-xl bg-primary font-bold text-white shadow-md hover:bg-indigo-500"
      ...
      <Dialog open={showCreateRoom} onOpenChange={setShowCreateRoom}>
        <DialogContent className="rounded-2xl border-border bg-card text-slate-100 sm:max-w-[420px]">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
                Invite Friends (by PIN)
              </Label>
              <div className="flex gap-2">
                <Input
                  className="h-11 flex-1 rounded-xl border-border bg-muted/60 font-mono text-white placeholder-slate-500 focus-visible:ring-indigo-500"
                  value={invitePin}
                  onChange={(e) =>
                ...
                <Button
                  variant="secondary"
                  className="hover:bg-slate-850 h-11 rounded-xl border border-border bg-muted px-4 text-xs font-semibold text-foreground"
              ...
              {invitedUsers.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {invitedUsers.map((u) => (
                    <span
                      key={u.id}
                      className="flex items-center gap-1 rounded-lg border border-indigo-500/20 bg-indigo-500/10 px-2 py-1 text-xs text-indigo-300"
                    >
                      ...
                      <button
                        ...
                        className="ml-0.5 text-primary hover:text-white"
                      >
                        &times;
                      </button>
                    </span>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <Input
                ...
                className="h-11 rounded-xl border-border bg-muted/60 pr-3 pl-9 text-white placeholder-slate-500 focus-visible:ring-indigo-500"
              />
            </div>
